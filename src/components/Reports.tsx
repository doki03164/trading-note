...
export function Reports() {
  ...
  return <section className="clone-page reports-page">
    ...
    <div className="report-grid"><article className="equity-card"><div><span>PERFORMANCE</span><strong>Cumulative equity curve</strong></div><div className="equity-curve">{curve.map((value, index) => <i key={index} className={value >= 0 ? 'positive-bar' : 'negative-bar'} style={{height:`${Math.max(Math.abs(value) / max * 85, 3)}%`}} title={`$${value.toFixed(2)}`}/>)}</div></article><article className="strategy-card"><div><span>PLAYBOOK REPORT</span><strong>Performance by strategy</strong></div>{byStrategy.map(([name, value]) => <div className="strategy-row" key={name}><span>{name}<small>{value.count} trades</small></span><strong className={value.pnl >= 0 ? 'positive' : 'negative'}>{value.pnl >= 0 ? '+' : '−'}${Math.abs(value.pnl).toFixed(2)}</strong></div>)}{!byStrategy.length && <div className="clone-empty">Add trades to generate reports.</div>}</article></div>
  </section>;
}
